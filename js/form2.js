...
var sLeftLabelClass = "small-12 medium-2 columns"
var sMiddleDivClass = "small-12 medium-10 columns"
var sRightDivClass  = "small-12 medium-5 columns"
...
var sBaseURL        = "https://www.lsitools.com"
    ...
    function returnInput(sType, sInputId, sInputName, sValue, bUseDeaultClass, sInputClass, sCustomProperty1Name, sCustomProperty1Val, sCustomProperty2Name, sCustomProperty2Val, sCustomProperty3Name, sCustomProperty3Val, sCustomProperty4Name, sCustomProperty4Val, sCustomProperty5Name, sCustomProperty5Val) {
        ...
        if (sType == "checkbox") {
            $oInput = $("<input>").prop("type", "checkbox").prop("id", sInputId).prop("name", sInputName).css("margin-right", ".3rem");
            $oWrapper = $("<label>")
            if (bUseDeaultClass) { $oWrapper.addClass("left").css("margin-right", "10px") }
            $oWrapper.append($oInput)
        }

        if (bUseDeaultClass) {
            if (sType == "submit") {
                $oInput.addClass("button medium")
    ...
    function addInput(sType, sParentId, sInputId, sInputName, sLabelText, sValue, sParentClass, sLeftClass, sMiddleClass, sRightClass, sInputClass, sCustomProperty1Name, sCustomProperty1Val, sCustomProperty2Name, sCustomProperty2Val, sCustomProperty3Name, sCustomProperty3Val, sCustomProperty4Name, sCustomProperty4Val, sCustomProperty5Name, sCustomProperty5Val) {
        ...
        var $oParent = $("<div>").addClass("row").prop("id", sDivId + iDivId)
        if (sParentClass != "") { $oParent.addClass(sParentClass) }

        var $oLeft = $("<label>").addClass(sLeftLabelClass).addClass("right-label").prop("for", sInputId).append(sLabelText).prop("id", sInputId + "-left")
...
//alert("iOnlyOn = " + iOnlyOn)
    ...
    function showReportData(data) {
        ...
        $("#orepeats").append( $("<option/>", {text: "Every Year", value: (1440 * 334) } )) //334 days = 480,960
            ...
            $oRow.append("<td class='text-nowrap'><a class='raduis secondary label btn-click-edit' href='#' id='" + event.id + "edit'>Edit</a> <a class='raduis secondary label' href='#' id='" + event.id + "history'>History</a></td>")
            $otBody.append($oRow)
        ...
        $("#oreport").append("<table id='oreporttablekey' class='table'><tr><td><s>Strikethrough</s> = Not Active</td></tr></table>")
        $("#oreport").append("<a class='button medium radius' href='#' onclick='showEditor(); return false;'>Add New Event</a>")
        $("#oloadergif").hide()
    }
    ...
    function showEditor() {
        ...
        addInput("select", "oform", "orequester", "requester", "Requester", iRequesterId, "", "", "large-5", "large-5")
        addInput("select", "oform", "oassignee", "assignee", "Assignee", iAssigneeId, "", "", "large-5", "large-5")
        ...
        $("#osubmitbuttons-middle")
            .append(returnInput("submit", "osubmit", "submit", "Submit", false, "button medium left radius").css("margin-right", "10px"))
            .append(returnInput("submit", "ocancel", "cancel", "Cancel", false, "button medium alert left radius"))
        ...
        $("#odate").fdatepicker({"setDate": sDate, minViewMode: 0})
